...
public class Handler implements RequestHandler<BandRequest, BandResponse>{
    ...
    public BandResponse handleRequest(BandRequest request, Context context) {
        ...
        try {
            ...
            List<Band> bands = this.read(uid, request, secret);

            if(bands.size()>=1){
                id = this.update(uid, request, bands.get(0),secret);
            }else{
                throw new Exception("There's no coincidence with your target to update ");
            }
        ...
        return new BandResponse(uid, id, "Success Update");
    ...
    public int update(String uid, BandRequest newBand, Band oldBand,Secret dbSecret) {
        String sql = "UPDATE bands SET band_id='" + oldBand.getBandId() + "',band_name='" + newBand.getBandName() + "',"
                + "band_genre='" + newBand.getBandGenre() + "', band_year_init= '" + newBand.getBandYearInit() + "' where band_id='" + oldBand.getBandId() + "';";
        int id = 0;
        try {
            SingleDBConnection con = SingleDBConnection.getInstance("bands_inventory", dbSecret.getUsername(), dbSecret.getPassword(), dbSecret.getHost(), dbSecret.getPort());
            if (con.open()) {
                ...
                logger.log("ERROR!!: No se pudo conectar con base de datos!");
            }
        } catch (ClassNotFoundException ex) {
            logger.log("ERROR no har conexión con el driver." + ex.getMessage());
        } catch (SQLException ex) {
            logger.log("ERROR!!: No se pudo realizar la sentecia sql: " + sql + " " + ex.getMessage());
        }
